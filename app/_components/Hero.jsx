import Image from 'next/image'
import React from 'react'

function Hero() {
  return (
    <div>
     <section className="bg-black lg:grid lg:h-screen lg:place-content-center">
  <div
    className="mx-auto w-screen max-w-screen-xl px-4 py-16 sm:px-6 sm:py-24 md:grid md:grid-cols-2 md:items-center md:gap-4 lg:px-8 lg:py-32"
  >
    <div className="max-w-prose text-left">
      <h1 className="text-4xl font-bold text-white sm:text-5xl">
      AI-powered 
        <strong  className="text-[#618ebe]"> personalized  course </strong>
        curation
      </h1>

      <p className="mt-4 text-base text-pretty text-white sm:text-lg/relaxed">
        Unlock personalized education with AI-driven course creation. Tailor your Learing journey to fit your goals and pace
      </p>

      <div className="mt-4 flex gap-4 sm:mt-6">
        <a
          className="inline-block rounded border border-[#618ebe]-600 bg-[#618ebe] px-5 py-3 font-medium text-white shadow-sm transition-colors hover:bg-[#dc9325]"
          href="#"
        >
          Get Started
        </a>

        <a
          className="inline-block rounded border border-gray-200 px-5 py-3 font-medium text-[#618ebe] shadow-sm transition-colors hover:bg-gray-50 hover:text-gray-900"
          href="#"
        >
          Learn More
        </a>
      </div>
    </div>
     {/* <div>
      <Image src={'/education.svg'} width={500} height={400} alt="education" />
     </div> */}
     <div className="relative w-[500px] h-[600px]">
  {/* Transparent Shadow */}
  <Image 
    src={'/education.svg'} 
    width={500} 
    height={600} 
    alt="education" 
    className="absolute top-0 left-0 w-full h-full filter drop-shadow-[0_0_40px_rgba(97,142,190,0.7)] "
  />

  {/* Main Image */}
  
</div>

  
  </div>
</section>
    </div>
  )
}

export default Hero
